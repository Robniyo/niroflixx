...
import { BookOpen, DollarSign, Clock, ArrowRight } from 'lucide-react';
...
export default function MyEnrollments() {
  ...
  const handlePayRemaining = (enrollment: any) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-secondary-50 via-white to-primary-50">
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {e.remainingBalance > 0 && (
                      <Button size="sm" onClick={() => handlePayRemaining(e)}>
                        <DollarSign className="w-3.5 h-3.5 mr-1" /> Pay Remaining
                      </Button>
                    )}
                  </div>
                </div>
      ...
      {selectedEnrollment && (
        <PaymentModal
          ...
          amount={selectedEnrollment.remainingBalance}
          title={selectedEnrollment.course?.title || 'Course'}
          enrollmentId={selectedEnrollment.id}
          maxAmount={selectedEnrollment.remainingBalance}
        />
      )}
    </div>
  );
