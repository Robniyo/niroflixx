            ...
            renderItem={(opp) => (
              <div className="px-2">
                <Link to={`/opportunities/${opp.id}`} className="group block">
                  <div className="bg-secondary-50 rounded-xl border border-secondary-100 p-5 h-full flex gap-4 hover:shadow-md transition-all">
                    {opp.coverImage ? (
                      <img src={opp.coverImage} alt={opp.title} className="w-16 h-16 object-contain rounded-lg bg-white flex-shrink-0" />
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="font-semibold text-secondary-900 mt-1.5 group-hover:text-primary-600 transition-colors truncate">{opp.title}</h3>
                      <p className="text-body-sm text-secondary-500 mt-0.5 truncate">{opp.organization}</p>
