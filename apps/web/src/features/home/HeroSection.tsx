            ...
            <div className="lg:hidden w-full mb-4">
              {!searchOpen ? (
                ...
                <div className="bg-white rounded-xl p-3 shadow-xl">
                  <div className="flex items-center gap-2 mb-2"><Search className="w-5 h-5 text-secondary-400 flex-shrink-0" /><input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Search..." className="flex-1 px-2 py-2 text-body border-none outline-none text-secondary-900 placeholder:text-secondary-400 min-w-0" autoFocus /></div>
                  <div className="flex gap-2"><button onClick={() => setSearchOpen(false)} className="flex-1 py-2 text-sm text-secondary-500 hover:text-secondary-700">Cancel</button><Button size="md" className="flex-1" onClick={handleSearch}>Search</Button></div>
                </div>
              )}
            </div>

            <div className="hidden lg:flex items-center gap-2 bg-white rounded-lg p-1.5 shadow-xl max-w-md mb-8">
              <Search className="w-5 h-5 text-secondary-400 ml-3 flex-shrink-0" /><input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Search courses, scholarships, jobs..." className="flex-1 px-2 py-2 text-body border-none outline-none text-secondary-900 placeholder:text-secondary-400" /><Button size="md" className="flex-shrink-0" onClick={handleSearch}>Search</Button>
            </div>

            <div className="flex flex-wrap gap-3 mb-8 md:mb-10">
              {!isAuthenticated && <Link to="/register"><Button size="lg" variant="secondary" rightIcon={<ArrowRight className="w-4 h-4" />} className="shadow-lg font-semibold">Get Started Free</Button></Link>}
