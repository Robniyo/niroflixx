...
export default function Footer() {
  ...
  const [socials, setSocials] = useState({
    github: 'https://github.com/Robniyo',
  ...
  const fetchSettings = async () => {
    try {
      ...
      results.forEach(({ key, value }) => {
        if (!value) return;
        if (['facebook','twitter','instagram','linkedin','youtube','github','tiktok','whatsapp'].includes(key)) {
          newSocials[key] = value;
        } else if (key === 'contactEmail') newContact.contactEmail = value;
        else if (key === 'contactPhone') newContact.contactPhone = value;
        else if (key === 'contactAddress') newContact.contactAddress = value;
      });
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 mb-12">
          ...
          <div className="col-span-2">
            <Link to="/" className="flex flex-col items-center gap-0.5 mb-4">
              <img src="/flogo.png" alt="Future Scholars" className="h-9 max-w-[160px] w-auto object-contain" />
            ...
            <div className="space-y-2 text-body-sm text-secondary-400">
              <a href={`mailto:${contactInfo.contactEmail}`} className="flex items-center gap-2 hover:text-white transition-colors">
                <Mail className="w-4 h-4" /> {contactInfo.contactEmail}
              </a>
              <a href={`tel:${contactInfo.contactPhone.replace(/\s/g, '')}`} className="flex items-center gap-2 hover:text-white transition-colors">
                <Phone className="w-4 h-4" /> {contactInfo.contactPhone}
              </a>
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4" /> {contactInfo.contactAddress}
              </div>
            </div>
          </div>
        ...
        <div className="border-t border-secondary-800 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {[
              ...
              { key: 'tiktok', href: socials.tiktok, icon: 'tiktok', label: 'TikTok' },
